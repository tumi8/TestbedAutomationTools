...
class BeefAPI:
    ...
    def login(self, username, password):
        try:
            ...
            r = requests.post(self.login_url, data=auth, verify=False)
            data = r.json()
    ...
    @property
    def dns(self):
        r = requests.get(self.dns_url, verify=False)
...
class ARE_Rule(object):
    ...
    def trigger(self):
        r = requests.get('{}/trigger/{}?token={}'.format(self.url, self.id, self.token), verify=False)
        return r.json()

    def delete(self):
        r = requests.get('{}/delete/{}?token={}'.format(self.url, self.id, self.token), verify=False)
        return r.json()
...
class ARE_Rules(object):
    ...
    def list(self):
        rules = []
        r = requests.get('{}/list/all?token={}'.format(self.url, self.token), verify=False)
        data = r.json()
        if (r.status_code == 200) and (data['success']):
    ...
    def add(self, rule_path):
        if rule_path.endswith('.json'):
            ...
            with open(rule_path, 'r') as rule:
                payload = rule.read()
                r = requests.post('{}/add?token={}'.format(self.url, self.token), data=payload, headers=headers, verify=False)
            return r.json()

    def trigger(self, rule_id):
        r = requests.get('{}/trigger/{}?token={}'.format(self.url, rule_id, self.token), verify=False)
        return r.json()

    def delete(self, rule_id):
        r = requests.get('{}/delete/{}?token={}'.format(self.url, rule_id, self.token), verify=False)
        return r.json()
...
class Module(object):
    ...
    @property
    def options(self):
        r = requests.get("{}/modules/{}?token={}".format(self.url, self.id, self.token), verify=False).json()
        return r['options']
...
class DNS_Rule(object):
    ...
    def delete(self):
        r = requests.delete("{}/dns/rule/{}?token={}".format(self.url, self.id, self.token), verify=False)
        return r.json()
...
class DNS(object):
    ...
    def ruleset(self):
        ruleset = []
        r = requests.get("{}/dns/ruleset?token={}".format(self.url, self.token), verify=False)
    ...
    def add(self, pattern, resource, response=[]):
        ...
        r = requests.post("{}/dns/rule?token={}".format(self.url, self.token), headers=headers, data=payload, verify=False)
        return r.json()

    def delete(self, rule_id):
        r = requests.delete("{}/dns/rule/{}?token={}".format(self.url, rule_id, self.token), verify=False)
        return r.json()
...
class Session(object):
    ...
    def multi_run(self, options={}):
        ...
        r = requests.post("{}/modules/multi_module?token={}".format(self.url, self.token), headers=headers, data=payload, verify=False)
        return r.json()
